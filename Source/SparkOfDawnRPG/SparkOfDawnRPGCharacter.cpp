...
//////////////////////////////////////////////////////////////////////////
// ASparkOfDawnRPGCharacter

ASparkOfDawnRPGCharacter::ASparkOfDawnRPGCharacter()
{
...
void ASparkOfDawnRPGCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAxis("MoveForward", this, &ASparkOfDawnRPGCharacter::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &ASparkOfDawnRPGCharacter::MoveRight);

	// We have 2 versions of the rotation bindings to handle different kinds of devices differently
	// "turn" handles devices that provide an absolute delta, such as a mouse.
	// "turnrate" is for devices that we choose to treat as a rate of change, such as an analog joystick
	PlayerInputComponent->BindAxis("Turn", this, &APawn::AddControllerYawInput);
	PlayerInputComponent->BindAxis("TurnRate", this, &ASparkOfDawnRPGCharacter::TurnAtRate);
	PlayerInputComponent->BindAxis("LookUp", this, &APawn::AddControllerPitchInput);
	PlayerInputComponent->BindAxis("LookUpRate", this, &ASparkOfDawnRPGCharacter::LookUpAtRate);

	// handle touch devices
	PlayerInputComponent->BindTouch(IE_Pressed, this, &ASparkOfDawnRPGCharacter::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &ASparkOfDawnRPGCharacter::TouchStopped);

	// VR headset functionality
	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &ASparkOfDawnRPGCharacter::OnResetVR);
}


void ASparkOfDawnRPGCharacter::OnResetVR()
{
	// If SparkOfDawnRPG is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in SparkOfDawnRPG.Build.cs is not automatically propagated
...
void ASparkOfDawnRPGCharacter::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
		Jump();
}

void ASparkOfDawnRPGCharacter::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
		StopJumping();
}

void ASparkOfDawnRPGCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void ASparkOfDawnRPGCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerPitchInput(Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

void ASparkOfDawnRPGCharacter::MoveForward(float Value)
{
...
void ASparkOfDawnRPGCharacter::MoveRight(float Value)
{
	if ( (Controller != nullptr) && (Value != 0.0f) )
	{
